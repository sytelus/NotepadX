...
{
	/// <summary>
	/// NotepadXDocument represents the entire NotepadX doc
	/// </summary>
	...
	public class NotepadXConfiguration
	{
		...
		public NotepadXConfiguration(string xmlTagName, string rootNodeId, string defaultRootNodeTitle, DocumentAuthorInformation authorInfo) : this(xmlTagName, rootNodeId, defaultRootNodeTitle)
		{
			m_authorInfo = authorInfo;
		}		
		public NotepadXConfiguration(string xmlTagName, string rootNodeId, string defaultRootNodeTitle)
		{
		...
		public NotepadXConfiguration(string xmlTagName, string defaultRootNodeTitle)
		{
			if (xmlTagName != null)
				m_xmlTagName = xmlTagName;
			m_defaultRootNoteTitle = defaultRootNodeTitle;		
		...
		public NotepadXConfiguration()
		{
			//leave vars to their default value		
		}
		
		public NotepadXConfiguration(DocumentAuthorInformation authorInfo)
		{
			m_authorInfo = authorInfo;
		}
		public NotepadXConfiguration(string xmlTagName, string defaultRootNodeTitle, DocumentAuthorInformation authorInfo) : this(xmlTagName, defaultRootNodeTitle)
		{
			m_authorInfo = authorInfo;
		}
	...
	public class NotepadXDocument
	{
		public event FileNameChangedDelegate FileNameChanged;
		private NotepadXConfiguration m_catNoteConfiguration;
		...
		public NotepadXConfiguration Configuration
		{
			get
			{
		...
		public string NoteBookmark
		{
			get
			{
				if (m_noteBookmark == null)
				{
					...
					{
						CatNoteDetail rootCatNoteDetail = this.GetNoteById(Configuration.RootNoteId);
		...
		public NotepadXDocument() : this(new NotepadXConfiguration(), false, true)
		{
		}

		public NotepadXDocument(NotepadXConfiguration thisNotepadXConfiguration) : this(thisNotepadXConfiguration, thisNotepadXConfiguration.IsDirty, true)
		{
		}

		private NotepadXDocument(NotepadXConfiguration thisNotepadXConfiguration, bool isDirty, bool makeNewDocument)
		{
		...
		private void InitializePrivateVars(NotepadXConfiguration thisNotepadXConfiguration)
		{
		...
		public NotepadXDocument(System.IO.Stream inStream) : this(inStream, new NotepadXConfiguration())
		{
		}		
		public NotepadXDocument(System.IO.Stream inStream, NotepadXConfiguration config) : this(config, false, false)
		{
			m_NotepadXXmlDocument.Load(inStream);
			m_catNoteConfiguration.SetIsDirty(false);
		}
		public NotepadXDocument(string fileName, NotepadXConfiguration config) : this(config, false, false)
		{
			LoadFromFile(fileName);
		}
		public NotepadXDocument(string fileName) : this(fileName, new NotepadXConfiguration())
		{
		}
		...
		public void MakeBlankDocument()
		{
			...
			m_catNoteConfiguration.IsDocumentReadOnly = false;
			CatNoteDetail rootNote = this.GetNoteById(this.Configuration.RootNoteId);
		...
		public NotepadXCollection GetChildNotes(string parentNoteId)
		{
			return GetChildNotes(parentNoteId, true);
		}
		public NotepadXCollection GetChildNotes(string parentNoteId, bool isSearchRecursively)
		{
			...
			{
				return new NotepadXCollection(parentXmlNode, m_catNoteConfiguration);
			}
		}

		public CatNoteDetail CreateNoteInRoot(string noteTitle)
		{
			NotepadXCollection rootChilds = GetChildNotes(Configuration.RootNoteId, false);
			return rootChilds.Add(noteTitle);
		}
		
		public NotepadXCollection GetChildNotesByNoteTitle(string noteTitle)
		{
			return GetChildNotesByNoteTitle(noteTitle, true);
		}

		public NotepadXCollection GetChildNotesByNoteTitle(string noteTitle, bool isSearchRecursively)
		{
			XmlNode parentXmlNode;
		...
		public CatNoteDetail GetNoteById(string noteId)
		{
			...
			{
				return new CatNoteDetail(catNoteNode, m_catNoteConfiguration);
			}
		}
		...
		public void CopyNoteByContent(string outerXmlForNote, string destinationNoteId)
		{
			if (m_catNoteConfiguration.IsDocumentReadOnly == false)
			{
				NotepadXCollection childNotes = GetChildNotes(destinationNoteId);
		...
		{
			CatNoteDetail noteToChange = GetNoteById(noteId);
			...
			{
				NotepadXCollection childNotes = GetChildNotes(noteToChange.Id);
				if (childNotes != null)
				{
					foreach (DictionaryEntry childNoteDictionaryEntry in childNotes)
					{
						CatNoteDetail childCatNote = ((CatNoteDetail)childNoteDictionaryEntry.Value);
		...
		public CatNoteDetail GetNoteByTitle(string noteTitle)
		{
	...
	/// <summary>
	/// Collection of CatNoteDetail
	/// </summary>
	public class NotepadXCollection : System.Collections.DictionaryBase 
	{
		XmlNode m_NotepadXDocumentParentXmlNode;
		NotepadXConfiguration m_catNoteConfiguration;
		
		protected internal NotepadXCollection(XmlNode NotepadXParentXmlNode, NotepadXConfiguration thisNotepadXConfiguration)
		{
			...
			{
				CatNoteDetail newNoteForCollection =new CatNoteDetail(catNoteDetailNode, m_catNoteConfiguration);
		...
		public CatNoteDetail this[string noteId]
		{
			get
			{
				return (CatNoteDetail) base.Dictionary[noteId];
			}
		}
		
		public CatNoteDetail GetNoteDetailByNoteTitle(string noteTitle)
		{
			return GetNoteDetailByNoteTitle(noteTitle, true);
		}
		
		public CatNoteDetail GetNoteDetailByNoteTitle(string noteTitle, bool isSearchRecursively)
		{
			string xpathCriteriaPrefix = (isSearchRecursively? @"//" : "./");
			...
			{
				return new CatNoteDetail(foundNoteDetail, m_catNoteConfiguration);
			}
			else return null;
		}
		...
		protected internal string AddXmlDirect(string outerXmlForNote)
		{
			...
			CatNoteDetail newNote = new CatNoteDetail(newNoteXmlFragment.FirstChild, m_catNoteConfiguration);
			string newId = Guid.NewGuid().ToString();
			newNote.SetId (newId);
		...
		public CatNoteDetail Add()
		{
			return this.Add("");
		}
		
		public CatNoteDetail Add(string noteTitle)
		{
			if (m_catNoteConfiguration.IsDocumentReadOnly == false)
			{
				CatNoteDetail newNoteForCollection = CatNoteDetail.CreateCatNoteDetail(m_NotepadXDocumentParentXmlNode, m_catNoteConfiguration);
		...
		public void Remove(string catNoteId)
		{
			if (m_catNoteConfiguration.IsDocumentReadOnly == false)
			{
				CatNoteDetail catNoteDetailForId = (CatNoteDetail) Dictionary[catNoteId];
	...
	public class CatNoteDetail
	{
		...
		protected internal CatNoteDetail(XmlNode catNoteDetailXmlNode, NotepadXConfiguration thisNotepadXConfiguration)
		{
			m_catNoteConfiguration = thisNotepadXConfiguration;
		...
		protected internal static CatNoteDetail CreateCatNoteDetail(XmlNode parentXmlNode, NotepadXConfiguration thisNotepadXConfiguration)
		{
			...
			return new CatNoteDetail(newCatNoteDetailXmlNode, thisNotepadXConfiguration);
		}
		...
		/// <summary>
		/// CatNoteDetail class to access note's content and other attributes
		/// </summary>
		public CatNoteDetail ParentNote
		{
			get
			{
				if (m_catNoteDetailXmlNode.ParentNode != null)
					return new CatNoteDetail(m_catNoteDetailXmlNode.ParentNode, m_catNoteConfiguration);
				else
					return null;
			}
		}
		...
		public NotepadXCollection GetChildNotes()
		{
			return new NotepadXCollection(m_catNoteDetailXmlNode, m_catNoteConfiguration);
		}
		...
		public string Format
		{
			get
			{
				return GetAttributeValue("format", NoteFormat.FORMAT_TEXT);
			}
			set
			{
