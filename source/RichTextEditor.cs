...
namespace Sytel.NotepadX
{
	...
	/// </summary>
	public class RichTextEditor : System.Windows.Forms.UserControl, NotepadX.INoteEditor 
	{
		...
		{
			if (m_currentFormat == NoteFormat.FORMAT_RTF)
		...
		void INoteEditor.SetNoteContent(string noteID, string noteCaption, string noteContent)
		{
			...
			SetupUIForFormat(CurrentFormat);
		}
		...
		public string CurrentFormat
		{
			...
			set
			{
				if ((value == NoteFormat.FORMAT_RTF) || (value == NoteFormat.FORMAT_TEXT))
				{
		...
		private void SetRTFValueSafe(string rtfValue)
		{
			...
			catch (ArgumentException ex)	//Invalid RTF format
			{
		...
		{
			bool isPlainTextFormat = (currentFormat == NoteFormat.FORMAT_TEXT);
