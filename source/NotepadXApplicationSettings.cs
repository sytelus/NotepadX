...
{
	/// <summary>
	/// Summary description for NotepadXApplicationSettings.
	/// </summary>
	public class NotepadXApplicationSettings
	{
		...
		public NotepadXApplicationSettings() : this(Path.GetFileName(Application.ExecutablePath) + ".config.npx")
		{
		}

		public NotepadXApplicationSettings(string settingsFileNameToUse)
		{
			m_NotepadXDocument = null;
			m_NotepadXDocument = new NotepadXDocument(new NotepadXConfiguration(null, "Categorial configuration file for product: " + Path.GetFileName(Application.ExecutablePath)));
...
#region "usage statistics function"
		...
		{	
			NotepadXCollection settingsInCategory = GetCategoryForSetting(settingCategoryName, false);
			
			if (settingsInCategory != null)
			{
				CatNoteDetail settinngDetail = settingsInCategory.GetNoteDetailByNoteTitle(settingName, false);
		...
		private NotepadXCollection GetCategoryForSetting(string settingCategoryName, bool createCategoryIfNotExist)
		{
			NotepadXCollection settingsInCategory = null;;
			NotepadXCollection currentConfigurationSetCollection = null;
		...
		{
			NotepadXCollection settingsInCategory = GetCategoryForSetting(settingCategoryName, true);
